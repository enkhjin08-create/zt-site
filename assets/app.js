...
function emptyBox(){
  return { recipient: null, containerId: null, itemIds: [], message: "", customerName: "", customerPhone: "", customerEmail: "", deliveryDistrict: "", deliveryKhoroo: "", deliveryAddress: "", couponCode: "", couponType: null, couponValue: 0, orderNumber: null };
}
function loadBox(){
  try{
...
function productArtHTML(p, opts){
  ...
  const artClass = opts.detail ? "product-art is-detail" : "product-art";
  if(p.image){
    return `<div class="${artClass}" style="background:${cat.tint}">
        ${badge}${starBadge}
        <img src="${p.image}" alt="${escapeHTML(p.name)}" loading="lazy" style="width:100%;height:100%;object-fit:${opts.detail?'contain':'cover'};object-position:center ${opts.detail?'':'25%'};${opts.detail?'padding:8px':''}">
      </div>`;
  }
